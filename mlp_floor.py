...
def KFold(k, und_df_phone):
	...
	activations = ['identity', 'logistic', 'tanh', 'relu']
	max_its = [200,400,600]
	mlp_floor = GridSearchCV(estimator=model, param_grid=dict(solver = solvers,activation =activations,max_iter=max_its),n_jobs=4) #GRID
	...
	for i in range(k):
		...
		#Training		
		mlp_floor.fit(data_train,target_train_floor)
		...
		for j in range(len(und_df_phone)):
			...
			hit_rate_floor_mlp[j].append(mlp_floor.score(data_test , target_test_floor))

	np.save("hit_rate_floor_mlp.npy",hit_rate_floor_mlp)		
	print "hit rate floor mlp"
	print str(np.mean(hit_rate_floor_mlp[0])) + " - " +  str(np.std(hit_rate_floor_mlp[0]))
	print str(np.mean(hit_rate_floor_mlp[1])) + " - " +  str(np.std(hit_rate_floor_mlp[1]))
	print str(np.mean(hit_rate_floor_mlp[2])) + " - " +  str(np.std(hit_rate_floor_mlp[2]))
	print str(np.mean(hit_rate_floor_mlp[3])) + " - " +  str(np.std(hit_rate_floor_mlp[3]))
	print " "			

	print "Best Params"
	print mlp_floor.best_params_
