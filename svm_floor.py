...
def KFold(k, und_df_phone):
	...
	degrees = [3,5,7]
	Cs = [0.1,1,10,100]
	svm_floor= GridSearchCV(estimator=model, param_grid=dict(kernel=kernels,degree = degrees,C = Cs),n_jobs=4) #GRID
	...
	for i in range(k):
		...
		target_train_floor= train['FLOOR']
		...
		for j in range(len(und_df_phone)):
			...
			target_test_floor = test[j]['FLOOR']
