...
def KFold(k, und_df_phone):
	...
	activations = ['identity', 'logistic', 'tanh', 'relu']
	max_its = [200,400,600]
	mlp_building = GridSearchCV(estimator=model, param_grid=dict(solver = solvers,activation =activations,max_iter=max_its),n_jobs=4) #GRID
	...
	for i in range(k):
		...
		#Training		
		mlp_building.fit(data_train,target_train_build)
		...
		for j in range(len(und_df_phone)):
			...
			hit_rate_build_mlp[j].append(mlp_building.score(data_test , target_test_build))

	np.save("hit_rate_build_mlp.npy",hit_rate_build_mlp)		
	print "hit rate building mlp"
	print str(np.mean(hit_rate_build_mlp[0])) + " - " +  str(np.std(hit_rate_build_mlp[0]))
	print str(np.mean(hit_rate_build_mlp[1])) + " - " +  str(np.std(hit_rate_build_mlp[1]))
	print str(np.mean(hit_rate_build_mlp[2])) + " - " +  str(np.std(hit_rate_build_mlp[2]))
	print str(np.mean(hit_rate_build_mlp[3])) + " - " +  str(np.std(hit_rate_build_mlp[3]))
	print " "			

	print "Best Params"
	print mlp_building.best_params_
