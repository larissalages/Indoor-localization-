...
def KFold(k, und_df_phone):
	...
	Cs = [0.1,1,10,100]
	svm_building= GridSearchCV(estimator=model, param_grid=dict(C = Cs),n_jobs=6) #GRID
	svm_floor = GridSearchCV(estimator=model, param_grid=dict(C = Cs),n_jobs=6) #GRID
...
#	svm_floor = SVC()
	...
	hit_rate_floor_svm = init_list_of_objects(len(und_df_phone))
	real_hit_rate_svm = init_list_of_objects(len(und_df_phone))
	...
	for i in range(k):
		...
		svm_floor.fit(data_train,target_train_floor)
		...
		for j in range(len(und_df_phone)):
			...
			predictions_build = svm_building.predict(data_test).tolist()
			list_predictions_build[j].append(predictions_build) 

			hit_rate_floor_svm[j].append(svm_floor.score(data_test , target_test_floor))
			predictions_floor = svm_floor.predict(data_test).tolist()
			...
			real_hit_rate_svm[j].append( successful_amount/float(len(data_test)) )
			
	#saving data
	np.save("list_predictions_floor_svm.npy",list_predictions_floor)
	np.save("list_predictions_build_svm.npy",list_predictions_build)
	np.save("hit_rate_build_svm.npy",hit_rate_build_svm)
	np.save("hit_rate_floor_svm.npy",hit_rate_floor_svm)
	np.save("real_hit_rate_svm.npy",real_hit_rate_svm)
	...
	print str(np.mean(hit_rate_floor_svm[0])) + " - " +  str(np.std(hit_rate_floor_svm[0]))
	print str(np.mean(hit_rate_floor_svm[1])) + " - " +  str(np.std(hit_rate_floor_svm[1]))
	print str(np.mean(hit_rate_floor_svm[2])) + " - " +  str(np.std(hit_rate_floor_svm[2]))
	print str(np.mean(hit_rate_floor_svm[3])) + " - " +  str(np.std(hit_rate_floor_svm[3]))
	print " "			
