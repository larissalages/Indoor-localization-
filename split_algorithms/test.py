...
def KFold(k, und_df_phone):
	...
	ks = [1,3,5,7,10]
	knn_building= GridSearchCV(estimator=model, param_grid=dict(n_neighbors = ks),n_jobs=2) #GRID
	knn_floor = GridSearchCV(estimator=model, param_grid=dict(n_neighbors = ks),n_jobs=2) #GRID	
#	knn_building= KNeighborsClassifier(n_neighbors=5, weights = 'distance')
#	knn_floor = KNeighborsClassifier(n_neighbors=5, weights = 'distance')
	...
	real_hit_rate_knn = init_list_of_objects(len(und_df_phone))
	...
	for i in range(k):
		...
		#Training		
		knn_building.fit(data_train,target_train_build)
		knn_floor.fit(data_train,target_train_floor)
		...
		for j in range(len(und_df_phone)):
			...
			hit_rate_build_knn[j].append(knn_building.score(data_test , target_test_build))
			predictions_build = knn_building.predict(data_test).tolist()
			list_predictions_build[j].append(predictions_build) 

			hit_rate_floor_knn[j].append(knn_floor.score(data_test , target_test_floor))
			predictions_floor = knn_floor.predict(data_test).tolist()
			...
			real_hit_rate_knn[j].append( successful_amount/float(len(data_test)) )
			
	#saving data
	np.save("list_predictions_floor_rf.npy",list_predictions_floor)
	np.save("list_predictions_build_rf.npy",list_predictions_build)
	np.save("hit_rate_build_knn.npy",hit_rate_build_knn)
	np.save("hit_rate_floor_knn.npy",hit_rate_floor_knn)
	np.save("real_hit_rate_knn.npy",real_hit_rate_knn)

	print "hit rate building KNN"
	print str(np.mean(hit_rate_build_knn[0])) + " - " +  str(np.std(hit_rate_build_knn[0]))
	print str(np.mean(hit_rate_build_knn[1])) + " - " +  str(np.std(hit_rate_build_knn[1]))
	print str(np.mean(hit_rate_build_knn[2])) + " - " +  str(np.std(hit_rate_build_knn[2]))
	print str(np.mean(hit_rate_build_knn[3])) + " - " +  str(np.std(hit_rate_build_knn[3]))
	print " "			

		
	print "hit rate floor KNN"
	...
	print knn_building.best_params_


	print "Best Params"
	print knn_floor.best_params_
